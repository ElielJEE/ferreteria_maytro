...
export async function GET(req) {
	try {
		...
		let sql = `SELECT ID_PROVEEDOR AS id, NOMBRE_PROVEEDOR AS nombre, TELEFONO_PROVEEDOR AS telefono FROM PROVEEDOR`;
		const params = [];
		if (q) {
			sql += ` WHERE NOMBRE_PROVEEDOR LIKE ? OR TELEFONO_PROVEEDOR LIKE ?`;
			params.push(`%${q}%`, `%${q}%`);
		}
		...
		const proveedores = (rows || []).map(r => ({ id: r.id, nombre: r.nombre || '', telefono: r.telefono || '' }));
