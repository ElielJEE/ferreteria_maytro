...
async function getOrCreateCliente(conn, nombre, telefono) {
  ...
  const values = [];
  if (name) {
    clauses.push("NOMBRE_CLIENTE = ?");
    values.push(name);
  }
  if (tel) {
    clauses.push("TELEFONO_CLIENTE = ?");
    values.push(tel);
  }
  const [rows] = await conn.query(
    `SELECT ID_CLIENTES FROM clientes WHERE ${clauses.join(" OR ")} LIMIT 1`,
    values,
  );
...
async function decrementStockForFactura(
  conn,
  facturaId,
  sucursalId,
  usuarioId = null,
  ...
  for (const detalle of detalles) {
    ...
    const stockAnterior = stockRows.length
      ? Number(stockRows[0].CANTIDAD || 0)
      : 0;
    if (totalARestar > stockAnterior) {
      throw new Error(
        `Stock insuficiente para el producto ${prodId} en sucursal ${sucursalId}`,
      );
    }
    const stockNuevo = stockAnterior - totalARestar;
    await conn.query(
      "UPDATE stock_sucursal SET CANTIDAD = ? WHERE ID_PRODUCT = ? AND ID_SUCURSAL = ?",
      [stockNuevo, prodId, sucursalId],
    );
    try {
      await conn.query(
...
export async function POST(req) {
  ...
  try {
    ...
    const payloadSucursalName =
      body?.sucursal ??
      body?.sucursal_name ??
      body?.sucursalNombre ??
      body?.sucursal_nombre ??
      null;
    ...
    try {
      if (!sucursalId && usuarioId) {
        const [uRows] = await conn.query(
          "SELECT ID_SUCURSAL FROM usuarios WHERE ID = ? LIMIT 1",
          [usuarioId],
        );
        if (uRows && uRows[0] && uRows[0].ID_SUCURSAL)
          sucursalId = uRows[0].ID_SUCURSAL;
      }
      // As an additional fallback, try by sucursal name in payload
      if (!sucursalId && payloadSucursalName) {
        const [suc] = await conn.query(
          "SELECT ID_SUCURSAL FROM sucursal WHERE NOMBRE_SUCURSAL = ? LIMIT 1",
          [payloadSucursalName],
        );
        if (suc && suc[0] && suc[0].ID_SUCURSAL)
          sucursalId = suc[0].ID_SUCURSAL;
      }
    } catch {
    ...
    let hasUnidadCols = {
      UNIDAD_ID: false,
      CANTIDAD_POR_UNIDAD: false,
      UNIDAD_NOMBRE: false,
    };
    try {
      const [cols] = await conn.query(
        `SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'factura_detalles'`,
      );
      const colset = new Set(
        (cols || []).map((r) => String(r.COLUMN_NAME).toUpperCase()),
      );
    ...
    const subtotalOk = Number.isFinite(Number(subtotal))
      ? Number(subtotal)
      : computedSubtotal;
    const descuentoOk = Number(descuento || 0);
    const servicioTrans =
      Number(
        (body?.servicio_transporte ??
          body?.servicioTransporte ??
          servicio_transporte) ||
          0,
      ) || 0;
    const totalOk = Number.isFinite(Number(total))
      ? Number(total)
      : Math.max(0, subtotalOk - descuentoOk + servicioTrans);
    ...
    if (hasFacturaNumero) {
      ...
      while (intentos < 5) {
        const [dup] = await conn.query(
          "SELECT 1 FROM factura WHERE NUMERO_FACTURA = ? LIMIT 1",
          [numeroFactura],
        );
        if (!dup?.length) break;
        intentos++;
    ...
    if (sucursalId) {
      try {
        await conn.query(
          "UPDATE factura SET ID_SUCURSAL = ? WHERE ID_FACTURA = ?",
          [sucursalId, facturaId],
        );
      } catch {
        /* ignore if column is absent or update fails */
      }
    }
    ...
    try {
      ...
      if (discountPayload) {
        ...
        `);
        await conn.query(
          "INSERT INTO factura_descuento (ID_FACTURA, ID_DESCUENTO, PERCENT, AMOUNT) VALUES (?, ?, ?, ?)",
          [facturaId, discId, percent, amount],
        );
      }
    } catch (err) {
    ...
    for (const it of items) {
      ...
      let unidadNombre =
        it.unit_name ?? it.unidad_nombre ?? it.UNIDAD_NOMBRE ?? null;
      if (unidadNombre) unidadNombre = String(unidadNombre).trim() || null;

      console.log(
        `[POST VENTAS] ANTES Producto ${idProd}: unidadId=${unidadId}, unidadNombre=${unidadNombre}, qty=${qty}`,
      );
      ...
      if (!unidadId || !unidadNombre) {
        try {
          ...
          console.log(
            `[POST VENTAS] Búsqueda en producto_unidades para ${idProd}:`,
            puRows,
          );
          ...
          if (Array.isArray(puRows) && puRows.length > 0) {
            ...
            const colValue =
              puRow.UNIDAD_ID ?? puRow.unidad_id ?? puRow.unidadId ?? null;
              ...
              try {
                ...
                if (Array.isArray(nameRows) && nameRows.length > 0) {
                  ...
                  if (nameValue) {
                    ...
                    console.log(
                      `[POST VENTAS] NOMBRE encontrado: ${unidadNombre}`,
                    );
                  }
                }
              } catch (nameErr) {
                console.error(
                  `[POST VENTAS] Error buscando nombre unidad ${unidadId}:`,
                  nameErr?.message,
                );
              }
            }
            ...
            console.warn(
              `[POST VENTAS] No hay producto_unidades para producto ${idProd}`,
            );
          }
        } catch (e) {
          console.error(
            "[POST VENTAS] Error buscando unidad:",
            e?.message,
            e?.stack,
          );
        }
      ...
      console.log(
        `[POST VENTAS] DESPUÉS Producto ${idProd}: unidadId=${unidadId}, unidadNombre=${unidadNombre}`,
      );
    ...
    try {
      if (!tasaCambio || isNaN(tasaCambio) || tasaCambio <= 0) {
        const [cfg] = await conn.query(
          "SELECT TASA FROM config_tasa_cambio WHERE ID = 1 LIMIT 1",
        );
    ...
    try {
      ...
      /* Tabla no existe aún; ignorar para compatibilidad */
    }
    ...
    return Response.json({
      ok: true,
      facturaId,
      numero: hasFacturaNumero ? numeroFactura : null,
      total: totalOk,
      cambio,
    });
  } catch (e) {
...
export async function GET(req) {
  try {
    ...
    try {
      ...
      if (token) {
        ...
        `,
          ...
          if (uRow) {
            usuarioSucursalId = uRow.ID_SUCURSAL || null;
            usuarioRol = (uRow.ROL || "").toString().trim().toLowerCase();
          }
        }
      }
    } catch {
      usuarioSucursalId = null;
      usuarioRol = null;
    }
    ...
    if (id) {
      try {
        ...
        const [clientRows] = await pool.query(
          "SELECT ID_CLIENTES, NOMBRE_CLIENTE, TELEFONO_CLIENTE FROM clientes WHERE ID_CLIENTES = ?",
          [f.ID_CLIENTES || null],
        );
        const cliente =
          clientRows && clientRows[0]
            ? {
                id: clientRows[0].ID_CLIENTES,
                nombre: clientRows[0].NOMBRE_CLIENTE,
                telefono: clientRows[0].TELEFONO_CLIENTE,
              }
            : null;
        ...
        if (f.ID_SUCURSAL) {
          const [sucRows] = await pool.query(
            "SELECT ID_SUCURSAL, NOMBRE_SUCURSAL FROM sucursal WHERE ID_SUCURSAL = ?",
            [f.ID_SUCURSAL],
          );
          if (sucRows && sucRows[0])
            sucursal = {
              id: sucRows[0].ID_SUCURSAL,
              nombre: sucRows[0].NOMBRE_SUCURSAL,
            };
        }
        ...
        const usuario =
          userRows && userRows[0]
            ? { id: userRows[0].ID, nombre: userRows[0].usuario }
            : null;
        ...
        const transporteValue = Number(
          f.SERVICIO_TRANSPORTE ??
            f.servicio_transporte ??
            f.servicioTransporte ??
            f.transporte ??
            0,
        );
        const facturaObj = {
          ...
          items: (itemsRows || []).map((it) => ({
            ...
            cantidad_por_unidad: Number(
              it.cantidad_por_unidad ?? it.CANTIDAD_POR_UNIDAD ?? 1,
            ),
        ...
        try {
          ...
          /* ignore additional discount read errors */
        }

        return Response.json({ factura: facturaObj });
      } catch (e) {
        return Response.json(
          { error: e.message || "Error al obtener detalle" },
          { status: 500 },
        );
      }
    }
    ...
    try {
      ...
      return Response.json(
        { error: e.message || "Error al obtener ventas" },
        { status: 500 },
      );
    }
  } catch (err) {
    return Response.json(
      { error: err.message || "Error en GET ventas" },
      { status: 500 },
    );
  }
}
...
export async function PUT(req) {
  ...
  try {
    ...
    console.log(
      "[PUT VENTAS] Items recibidos:",
      JSON.stringify(items, null, 2),
    );
    if (items && items.length > 0) {
      console.log(
        "[PUT VENTAS] Primer item detallado:",
    ...
    if ((!Array.isArray(items) || items.length === 0) && pago) {
      ...
      const [factRows] = await conn.query(
        "SELECT * FROM factura WHERE ID_FACTURA = ? FOR UPDATE",
        [id],
      );
      if (!factRows || !factRows.length) {
        await conn.rollback();
      ...
      if (estado) {
        ...
        if (
          nuevoEstado.toLowerCase() === "confirmado" &&
          facturaEstadoActual.toLowerCase() !== "confirmado"
        ) {
          await decrementStockForFactura(
            conn,
            id,
            factura.ID_SUCURSAL || null,
            factura.ID_USUARIO || null,
            "Venta confirmada",
          );
        }
        try {
    ...
    const willConfirm =
      nuevoEstado && nuevoEstado.toLowerCase() === "confirmado";
    ...
    try {
      const [cols] = await conn.query(
        `SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'factura_detalles' AND COLUMN_NAME = 'CANTIDAD_POR_UNIDAD'`,
      );
      detalleHasCantidadPorUnidad = cols && cols.length > 0;
    } catch {
      detalleHasCantidadPorUnidad = false;
    }
    ...
    const defaultUsuarioId =
      prevDetalles && prevDetalles[0]
        ? prevDetalles[0].ID_USUARIO || null
        : null;
    if (wasConfirmed) {
      for (const pd of prevDetalles || []) {
        ...
        if (!prodId) continue;
        await conn.query(
          "UPDATE stock_sucursal SET CANTIDAD = CANTIDAD + ? WHERE ID_PRODUCT = ? AND ID_SUCURSAL = ?",
          [restoreQty, prodId, sucursalId],
        );
        try {
          await conn.query(
            `INSERT INTO movimientos_inventario (producto_id, sucursal_id, usuario_id, tipo_movimiento, cantidad, motivo, referencia_id, stock_anterior, stock_nuevo)
    ...
    for (const it of items) {
      ...
      if (!prodId || qty <= 0) {
        await conn.rollback();
        return Response.json(
          { error: "Item inválido en nuevos items" },
          { status: 400 },
        );
      }
      if (wasConfirmed || willConfirm) {
        ...
        const cantidadEnSucursal = stockRows.length
          ? Number(stockRows[0].CANTIDAD || 0)
          : 0;
        const nombreSucursal = stockRows.length
          ? stockRows[0].NOMBRE_SUCURSAL
        ...
        if (totalARestar > cantidadEnSucursal) {
          await conn.rollback();
          return Response.json(
            { error: `Stock insuficiente para producto ${nombreSucursal}` },
            { status: 400 },
          );
        }
      }
    ...
    const totalOk = Number.isFinite(Number(total))
      ? Number(total)
      : Math.max(0, subtotalOk - descuentoOk);
    ...
    if (wasConfirmed || willConfirm) {
      await decrementStockForFactura(
        ...
        sucursalId,
        defaultUsuarioId,
        wasConfirmed ? "Edición venta" : "Venta confirmada",
      );
    }
    ...
    if (clienteNombre || clienteTelefono) {
      clienteId = await getOrCreateCliente(
        conn,
        clienteNombre,
        clienteTelefono,
      );
    ...
    const servicioTrans =
      Number((body?.servicio_transporte ?? body?.servicioTransporte) || 0) || 0;
    ...
    await conn.query(
      `UPDATE factura SET ${updateFields.join(", ")} WHERE ID_FACTURA = ?`,
      updateParams,
    );
    ...
    try {
      ...
      if (discountPayload) {
        ...
        `);
        await conn.query("DELETE FROM factura_descuento WHERE ID_FACTURA = ?", [
          id,
        ]);
        await conn.query(
          "INSERT INTO factura_descuento (ID_FACTURA, ID_DESCUENTO, PERCENT, AMOUNT) VALUES (?, ?, ?, ?)",
          [id, discId, percent, amount],
        );
      } else {
        await conn.query("DELETE FROM factura_descuento WHERE ID_FACTURA = ?", [
          id,
      ...
      console.error(
        "Error actualizando factura_descuento:",
        err?.message || err,
      );
    }
...
export async function DELETE(req) {
  ...
  try {
    ...
    try {
      await conn.query("DELETE FROM factura_pagos WHERE ID_FACTURA = ?", [id]);
    } catch {}
    await conn.query("DELETE FROM factura_detalles WHERE ID_FACTURA = ?", [id]);
