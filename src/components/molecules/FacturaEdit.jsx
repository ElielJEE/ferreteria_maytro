  ...
  const [items, setItems] = useState(() =>
    Array.isArray(factura?.products)
      ? factura.products.map((it) => ({
          ...
          subtotal: Number(
            it.subtotal ?? Number(it.cantidad || 0) * Number(it.precio || 0),
          ),
  ...
  const handleSave = async () => {
    try {
      ...
      const subtotal = items.reduce(
        (acc, it) => acc + Number(it.unitPrice || 0) * Number(it.cantidad || 0),
        0,
      );
      const descuento = 0;
